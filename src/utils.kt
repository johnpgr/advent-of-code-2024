...
fun <Input, Output> getSuspendFunctionName(fn: suspend (Input) -> Output): String = fn.toString().split("function ")[1].split(" ")[0]
...
suspend fun <Output> solveSuspend(fn: suspend (String) -> Output, input: String, asserts: Output? = null) {
    val fnName = getSuspendFunctionName(fn)
    val inputData = readInput(input)
    val startTime = System.nanoTime()
